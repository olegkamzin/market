			...
			await items.forEach(el => {
				axios.post('https://api.shinpi.ru/kolobox/orders/', null, {
					...
					axios.get('https://api.shinpi.ru/product/' + el.shopSku)
						.then(async product => {
                            await bot.sendMessage(263739791, `✅ Новый заказ <code>${order_number}</code> на сумму ${product.data.price * el.count} ₽. Отгрузка: ${req.body.order.delivery.shipments[0].shipmentDate}\r\n\r\n • ${el.offerName} - ${el.count} шт.\r\n\r\n- Артикул: <code>${product.data.article}</code>\r\n- ID: <code>${el.offerId}</code>\r\n- Резерв: <code>${result.data.orders[0]}</code>\r\n- Оптовая цена: ${product.data.wholesale_price * el.count}`, {parse_mode: 'HTML'}).catch(error => console.log(error))
							await bot.sendMessage(106773824, `✅ Новый заказ <code>${order_number}</code> на сумму ${product.data.price * el.count} ₽. Отгрузка: ${req.body.order.delivery.shipments[0].shipmentDate}\r\n\r\n • ${el.offerName} - ${el.count} шт.\r\n\r\n- Артикул: <code>${product.data.article}</code>\r\n- ID: <code>${el.offerId}</code>\r\n- Резерв: <code>${result.data.orders[0]}</code>\r\n- Оптовая цена: ${product.data.wholesale_price * el.count}`, {parse_mode: 'HTML'}).catch(error => console.log(error))
						})
				}).catch(async error => {
