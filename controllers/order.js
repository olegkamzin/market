		...
		try {
			...
			await items.forEach(async el => {
				await axios.post('https://api.shinpi.ru/kolobox/orders/', null, {
					...
					await bot.sendMessage(263739791, `✅ Новый заказ <code>${order_number}</code> на сумму ${req.body.order.totalWithSubsidy} ₽. Отгрузка: ${req.body.order.delivery.shipments.shipmentDate}\r\n\r\n • ${el.offerName} - ${el.count} шт.\r\n\r\nРезерв оформлен - <code>${result.data.orders[0]}</code>`, { parse_mode: 'HTML' }).catch(error => console.log(error))
					await bot.sendMessage(106773824, `✅ Новый заказ <code>${order_number}</code> на сумму ${req.body.order.totalWithSubsidy} ₽. Отгрузка: ${req.body.order.delivery.shipments.shipmentDate}\r\n\r\n • ${el.offerName} - ${el.count} шт.\r\n\r\nРезерв оформлен - <code>${result.data.orders[0]}</code>`, { parse_mode: 'HTML' }).catch(error => console.log(error))
				}).catch(async error => {
